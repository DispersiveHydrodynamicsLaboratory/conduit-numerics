...
x0 = zmax/2;
for ampl = [1.5:0.5:3]
    for boxmax = 40% [20:20:100]
        vstretch = 15;
        t0 = z0/(2*Aplus);
        tvec        = linspace(0,tmax,floor(tmax*100));
        f = @(z) ones(size(z));
        ic_type = 'constant';
        [ Avec, Adot ]  = BC_BOX( tvec, t0 , 0, boxmax, ampl , 1, 5);
         g0 = @(t) interp1(tvec,Avec,t,'spline','extrap');
        dg0 = @(t) interp1(tvec,Adot,t,'spline','extrap');
     g1 = @(t) ones(size(t));
    dg1 = @(t) zeros(size(t));
    if periodic
        bc_type = 'periodic';
    else
        bc_type = ['time_dependent_box_width_',...
                   num2str(boxmax),'_amplitude_',num2str(ampl)];
    end

%% Create directory run will be saved to
data_dir = ['H:\MATLABdata\data\conduit_eqtn\',...
