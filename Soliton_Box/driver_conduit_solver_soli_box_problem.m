...
%% PDE Initial and Boundary Conditions
 ...
 ws = [50 100 200];
asolis = [3 5 7];
[AMPLS,WS,ASOLIS] = meshgrid(ampls,ws,asolis);

for runi = [9]%[3 6:9 18 27]
	ampl  = AMPLS(runi);
    w     = WS(runi);
    asoli = ASOLIS(runi);
        vstretch = 5;
...
if runi == 8
    tmax     = 800;    % Solver will run from t=0 to t=tmax
    zmax     = 1000;     % Solver will solve on domain z=0 to z=zmax
elseif runi == 9
    tmax     = 900;    % Solver will run from t=0 to t=tmax
    zmax     = 1400;     % Solver will solve on domain z=0 to z=zmax
end
